...
/**
 * The app-lock gate, shared by EVERY portal activity that shows private content — MainActivity,
 * CloudChatActivity and PortalCallActivity each host one. Covers the window with a lock overlay
 * and prompts to authenticate, and (while the lock is enabled and screen-sharing isn't allowed)
 * sets FLAG_SECURE so the app never leaks through screenshots or the recent-apps thumbnail. Without
 * this on every launchable activity, a chat opened straight from a notification would bypass the
 * lock entirely.
 */
public final class LockGate {
    ...
    private void applySecure() {
        boolean secure = AppLock.isEnabled(mAct) && !AppLock.allowScreenShare(mAct);
        if (secure) {
    ...
    private void prompt() {
        ...
        mPrompting = true;
        AppLock.authenticate(mAct, "Unlock Parlons Cloud", "Fingerprint or device PIN",
    ...
    private View buildOverlay() {
        ...
        t.setText("Parlons Cloud is locked");
        t.setTextSize(16);
