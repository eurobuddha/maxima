...
/**
 * The portal call screen — the app's CallActivity pixel language (avatar, name, status, round
 * action buttons; video = fullscreen remote + PiP local), driven by {@link PortalCallManager}.
 */
public final class PortalCallActivity extends Activity implements PortalCallManager.Listener {
    ...
    protected void onCreate(Bundle zState) {
        ...
        mLiveRow.addView(roundButton("End", 0xFFE0524D,
                v -> PortalCallManager.get(this).hangup()));
        mLiveRow.addView(gap());
        ...
        if (mVideoUi) {
            ...
            mRemoteView.init(PortalCallManager.get(this).eglContext(), null);
            ...
            mLocalView.init(PortalCallManager.get(this).eglContext(), null);
            ...
            mLiveRow.addView(roundButton("Flip", 0xFF3A3D42,
                    v -> PortalCallManager.get(this).switchCamera()));
            setContentView(frame);
            PortalCallManager.get(this).attachVideoSinks(mLocalView, mRemoteView);
        } else {
            setContentView(root);
        }
    ...
    @Override
    public void onCallState(PortalCallManager.State zState, String zPeerKey, String zReason) {
    ...
    private void renderState(PortalCallManager.State zState, String zReason) {
        mIncomingRow.setVisibility(View.GONE);
        mLiveRow.setVisibility(View.GONE);
        mTick.removeCallbacksAndMessages(null);
        switch (zState) {
            case INCOMING_RINGING:
                mStatus.setText(mVideoUi ? "Incoming video call" : "Incoming Parlons call");
    ...
    private void tickTimer() {
        int s = PortalCallManager.get(this).liveSeconds();
