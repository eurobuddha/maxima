...
/**
 * Deterministic token identicon — a native port of NFTwallet's {@code Identicon}
 * (itself the utxoWallet dapp's buildIdenticonSVG): a 5×5 grid mirrored on the
 * vertical axis, cells + hue derived from the tokenid's hex. Same tokenid → same
 * picture, computed locally (leaks nothing). Plus the built-in Minima glyph for
 * the native token. No network, no dependencies.
 */
public final class Identicon {
    ...
    public static Bitmap forToken(String tokenid, int px) {
        ...
        for (int r = 0; r < 5; r++) {
            for (int c = 0; c < 3; c++) {
                int idx = (r * 3 + c + 8) % 32;
                if ((Character.digit(hex.charAt(idx), 16) & 1) == 0) {
                    continue;
                }
                int mirror = (c == 0) ? 4 : (c == 1 ? 3 : 2);
                drawCell(cv, p, c, r, cell, pad);
                if (mirror != c) {
                    drawCell(cv, p, mirror, r, cell, pad);
                }
            }
        }
        return bmp;
