...
/**
 * Cloud and Desktop's shared durable one-time-signature use counter — the file-backed counterpart
 ...
 * (so even two FileKeyUses instances over the same files exclude each other), and
 ...
 */
public class FileKeyUses implements KeyUses {
    ...
    public FileKeyUses(File walletDir, String namespace) {
        walletDir.mkdirs();
    ...
    public int reserveNextUse(int keyIndex) {
        return underLocks(() -> {
            ...
            if (n == Integer.MAX_VALUE) throw new IllegalStateException("KeyUses: counter exhausted; refusing to wrap");
            int next = n + 1;
    ...
    public void recordExternalUses(int keyIndex, int uses) {
        if (uses < 0) throw new IllegalArgumentException("negative key-use counter");
        underLocks(() -> {
    ...
    public static Map<String, Integer> exportAll(File zWalletDir) {
        FileKeyUses k = new FileKeyUses(zWalletDir, "");
        synchronized (k.lock) {
    ...
    public static void importRaiseOnly(File zWalletDir, Map<String, Integer> zIncoming) {
        ...
        k.underLocks(() -> {
            for (Map.Entry<String, Integer> e : zIncoming.entrySet()) {
                ...
                if (incoming < 0) throw new IllegalArgumentException("negative imported key-use counter");
    ...
    private Properties load(File f) {
        ...
        try (java.io.FileInputStream in = new java.io.FileInputStream(f)) {
            ...
            for (String name : p.stringPropertyNames()) {
                if (!name.startsWith("uses_")) continue;
                hasCounter = true;
                if (Integer.parseInt(p.getProperty(name)) < 0)
                    throw new IOException("negative key-use counter");
            }
            if (!hasCounter) throw new IOException("existing counter file has no counters");
        } catch (Exception e) {
            throw new IllegalStateException("KeyUses: " + f + " exists but is unreadable or invalid — "
    ...
    private void writeProps(File f, Properties p, String what) {
        ...
        try (FileOutputStream out = new FileOutputStream(tmp)) {
            p.store(out, "parlons wallet key-uses — DO NOT EDIT (one-time-signature counter)");
            out.flush();
