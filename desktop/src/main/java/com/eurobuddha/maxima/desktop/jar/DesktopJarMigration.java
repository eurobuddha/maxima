 ...
 */
public final class DesktopJarMigration {
	...
	private DesktopJarMigration() {
	}

	public static boolean needed(java.io.File zDataDir) {
		return !java.util.prefs.Preferences.userRoot().node(PREFS_NODE)
				.getBoolean(PREF_DONE, false);
	}

	public static void markDone(java.io.File zDataDir) {
		java.util.prefs.Preferences prefs =
				java.util.prefs.Preferences.userRoot().node(PREFS_NODE);
		prefs.putBoolean(PREF_DONE, true);
		prefs.putLong("migrated_at", System.currentTimeMillis());
	}
	...
	public static boolean inHealWindow(java.io.File zDataDir) {
		long at = java.util.prefs.Preferences.userRoot().node(PREFS_NODE)
	 ...
	 */
	public static int exportContacts(java.io.File zDataDir, DesktopJarEngine zJar) {
		int n = 0;
		try {
			FileStore store = new FileStore(new File(zDataDir, "node"));
			...
			com.eurobuddha.maxima.desktop.ui.DesktopEventLog.add("engine sync: export failed: " + e);
		}
		return n;
	}

	public static void archiveOldJarData(java.io.File zDataDir) {
		File data = new File(zDataDir, "maxjar");
		if (data.exists()) {
			File aside = new File(zDataDir,
					"maxjar.pre-migration-" + System.currentTimeMillis());
			if (data.renameTo(aside)) {
				com.eurobuddha.maxima.desktop.ui.DesktopEventLog.add("migration: previous jar data archived to " + aside.getName());
			}
		}
	}
	 ...
	 */
	public static void seedIdentity(java.io.File zDataDir, MaximaIdentity zId) {
		try {
			MaximaIdentity id = zId;
			...
			com.eurobuddha.maxima.desktop.ui.DesktopEventLog.add("migration: identity carried over (" + id.publicKeyHex() + ")");
		} catch (Exception e) {
			...
			com.eurobuddha.maxima.desktop.ui.DesktopEventLog.add("migration: identity seed FAILED: " + e);
	 ...
	 */
	public static int seedContacts(java.io.File zDataDir, DesktopJarEngine zJar) {
		int n = 0;
		try {
			FileStore store = new FileStore(new File(zDataDir, "node"));
			...
			for (Map.Entry<String, String> e : store.all("contacts").entrySet()) {
				try {
					...
					if (primary == null || primary.isEmpty()) {
						com.eurobuddha.maxima.desktop.ui.DesktopEventLog.add("migration: skipped " + c.name + " (no address)");
						continue;
					}
					...
					com.eurobuddha.maxima.desktop.ui.DesktopEventLog.add("migration: bad contact record skipped: " + one);
				}
			}
		} catch (Exception e) {
			com.eurobuddha.maxima.desktop.ui.DesktopEventLog.add("migration: contact replay FAILED: " + e);
		}
		if (n > 0) {
			com.eurobuddha.maxima.desktop.ui.DesktopEventLog.add("migration: " + n + " contact(s) carried into the classic book");
		}
		return n;
	}
}
