...
echo "== dmg: $DMG"
codesign -dv "$DMG" 2>&1 | grep -q 'Developer ID' || { echo "FAIL: dmg not signed with Developer ID"; exit 1; }
...
echo "== app: $APP"
codesign -dv --verbose=2 "$APP" 2>&1 | grep -E 'Authority=Developer ID Application|flags=' || true
codesign -dv --verbose=2 "$APP" 2>&1 | grep -q 'Authority=Developer ID Application' || { echo "FAIL: app not signed with Developer ID Application"; exit 1; }
codesign -dv --verbose=2 "$APP" 2>&1 | grep -q 'flags=.*runtime' || { echo "FAIL: hardened runtime not enabled"; exit 1; }
