...
/**
 * Port of the utxoWallet dapp's resolveTokenIcon (via {@code apks/NFTwallet IconResolver}) — turns a
 * raw token url/icon string into something loadable (a data: URI or an http(s) URL), matching the
 * canonical Minima wallet shape:
 *  1. {@code <artimage>BASE64</artimage>} wrapper → data:image/*;base64  (the PRIMARY real-token format)
 *  2. data:image/* URI
 *  3. http(s):// URL
 *  4. inline &lt;svg&gt;…&lt;/svg&gt; → data:image/svg+xml;base64 (rasterised by ImageLoader via AndroidSVG)
...
public final class IconResolver {
    ...
    public static String resolve(String raw) {
        ...
        if (PCT.matcher(t).find()) {
            try {
                t = URLDecoder.decode(t, "UTF-8").trim();
        ...
        if (t.regionMatches(true, 0, "<artimage", 0, 9)) {
            ...
            if (m.find()) {
                ...
                if (b64.length() > 16) {
                    return ImageTools.dataUri(b64);
                }
            }
            // Tolerate an UNTERMINATED <artimage> prefix: earlier builds wrote the opening tag without
            // a closing one, and those tokens are sealed on-chain forever — forgive it or the art is lost.
            Matcher open = ARTIMAGE_OPEN.matcher(t);
            if (open.find()) {
                String b64 = t.substring(open.end()).replaceAll("\\s+", "");
                if (b64.length() > 16 && B64.matcher(b64).matches()) {
                    return ImageTools.dataUri(b64);
                }
            }
        }
        if (DATA_IMG.matcher(t).find()) {
            return t;
        }
        if (HTTP.matcher(t).find()) {
            return t;
        }
        if (t.regionMatches(true, 0, "<svg", 0, 4) && t.toLowerCase().endsWith("</svg>")) {
            try {
                return "data:image/svg+xml;base64," + Base64.encodeToString(t.getBytes("UTF-8"), Base64.NO_WRAP);
            } catch (Exception e) {
                return null;
            }
        }
        if (t.length() >= 100 && B64.matcher(t).matches()) {
